...
// *** Generation date: Mon Jun 09 17:19:32 CEST 2014 
// *** reqT version 3.0.0-snapshot build date Mon Jun 09 17:19:32 CEST 2014
/*** 
**                  _______        
**                 |__   __|       
...
object metamodel extends MetamodelTypes {
  ...
  lazy val contextEntities: Vector[EntityType] = Vector(Actor, Application, Domain, Module, Product, Release, Resource, Risk, Service, Stakeholder, System, User)   
  ...
  lazy val dataReqs: Vector[EntityType] = Vector(Class, Data, Member, Relationship)
  ...
  lazy val stringAttributes: Vector[StringType] = Vector(Comment, Deprecated, Example, Expectation, File, Gist, Image, Input, Output, Spec, Text, Title, Why)
  lazy val intAttributes: Vector[IntType] = Vector(Benefit, Capacity, Cost, Damage, Differentiation, Frequency, MinValue, MaxValue, Order, Prio, Profit, Saturation, Urgency, Utility, Value)
  lazy val cardinalityAttributes: Vector[CardinalityType] = Vector(Opt) 
  lazy val relationTypes: Vector[RelationType] = Vector(has, is, superOf) ++ Vector(binds, deprecates, excludes, helps, hurts, implements, preceedes, requires, relatesTo, verifies)
...
case class MinValue(value: Int) extends IntAttribute { override val myType = MinValue }
case object MinValue extends IntType 

case class MaxValue(value: Int) extends IntAttribute { override val myType = MaxValue }
case object MaxValue extends IntType 
...
case class Urgency(value: Int) extends IntAttribute { override val myType = Urgency }
case object Urgency extends IntType 
...
case object preceedes extends RelationType
...
trait RelationFactory {
  ...
  def preceedes(elems: Elem*) = Relation(this, reqT.preceedes, Model(elems:_*))
  def preceedes(submodel: Model) = Relation(this, reqT.preceedes, submodel)
...
trait HeadFactory {
  ...
  def preceedes = Head(this, reqT.preceedes)
...
trait HeadTypeFactory {
  ...
  def preceedes = HeadType(this, reqT.preceedes)
...
trait ImplicitFactoryObjects extends CanMakeAttr { //mixed in by package object reqT
  ...
  implicit object makeMinValue extends AttrMaker[MinValue] { def apply(s: String): MinValue = MinValue(s.toInt) }
  implicit object makeMaxValue extends AttrMaker[MaxValue] { def apply(s: String): MaxValue = MaxValue(s.toInt) }
  ...
  implicit object makeUrgency extends AttrMaker[Urgency] { def apply(s: String): Urgency = Urgency(s.toInt) }
  ...
  lazy val attributeFromString = Map[String, String => Attribute[_]](
    ...
    "MinValue" -> makeAttribute[MinValue] _ ,
    "MaxValue" -> makeAttribute[MaxValue] _ ,
