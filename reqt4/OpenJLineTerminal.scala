...
class OpenJLineTerminal extends java.io.Closeable {
...
// open class OpenJLineTerminal(val Prompt: String, val NewLinePrompt: String) 
...
//     if (ctx.settings.color.value != "never") Console.BLUE + str + Console.RESET
//     else str
//   private def prompt(using Context)        = blue(Prompt)
//   private def newLinePrompt(using Context) = blue(NewLinePrompt)
