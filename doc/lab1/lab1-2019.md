...
* Ordinal-scale prioritization: parwise comparison
    ...
    * Choose *"Proiritize: analyse comparisions"* in the **Tools** menu. You should get a model with priorities calculated as a solution to a constraint problem based on your comparisons. A higher prio value means higher priority (more important). Write down reflections on the result:
        ...
        * Circular contradictions make the constraint problem unsolvable, but the constraint solver in reqT does not give up; instead a *relaxed deviation* `d` is allowed starting with `d = 1` and the retrying with `d += 1` until a solution can be found. This means that each priority rank `r` is representing any value within an error margin range of `[r - d, r + d]`. Given the minimum deviation needed for a solution to exist with your contradictory comparisons: find some features in your resulting priority rank order that could have be swapped and still fulfill the constraints if the deviation `d` is allowed.
